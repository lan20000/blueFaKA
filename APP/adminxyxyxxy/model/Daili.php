...
class Daili extends Model
{
     ...
     {
        if(!$data['password']&&!$data['payment']){
            $res=$this->where(['id'=>$data['id']])->update(['email'=>$data['email'],'money'=>$data['money']]);
        }else{
            $res=$this->where(['id'=>$data['id']])->update(['email'=>$data['email'],'money'=>$data['money'],'password'=>md5(md5($data['password'])),'paypassword'=>md5(md5($data['payment'])) ]);
        }
        return $res;
     }
     public function adddaili($data)
     {
         if(!$data['username']||!$data['email']){
             return ['vaild'=>0,'msg'=>'用户或者邮箱不得为空'];exit;
         }
         $dlif=$this->where(['username'=>$data['username']])->find();
         if($dlif){
             return ['vaild'=>0,'msg'=>'用户名已存在'];exit;
         }
            $res=$this;
            ...
            if($res){
                return ['vaild'=>1,'msg'=>'添加成功'];exit;
            }else{
                return ['vaild'=>0,'msg'=>'添加失败'];exit;
            }
     }
