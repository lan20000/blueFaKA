...
class Search extends Model
{
    ...
    {
       $ddinfo=$this->where(['ddid'=>$ddid,'status'=>1])->whereOr(['email'=>$ddid,'status'=>1])->order('id desc')->limit(1)->select();
       return $ddinfo;
    }
    public function countorder($ddid){
        $count=$this->where(['ddid'=>$ddid,'status'=>1])->order('id desc')->limit(1)->select();
        if(empty($count))
        {
        	$count=$this->where(['email'=>$ddid,'status'=>1])->order('id desc')->limit(1)->select();
        }
        if(empty($count))
        {
        	$count=$this->where(['payno'=>$ddid,'status'=>1])->order('id desc')->limit(1)->select();
        }		
        return $count;
    }
